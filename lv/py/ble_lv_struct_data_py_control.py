...
async def _handle_fusion_settings(cmd: int, doubles: list[float]):
    if cmd == 0x01 and len(doubles) == 3:
        ...
        # 处理温控设置
        VI.SetControlValue("setpoint", doubles[0])
        VI.SetControlValue("警告温度上限", doubles[1])
        VI.SetControlValue("警告温度下限", doubles[2])
    elif cmd == 0x02 and len(doubles) == 8:
        logging.info(f"[FusionSettings] 保存 PID 设置: {doubles}")
        # 处理 PID 设置
        VI.SetControlValue("output range", (doubles[0], doubles[1]))
        VI.SetControlValue("PID gains", (doubles[2], doubles[3], doubles[4]))
        VI.SetControlValue("温度误差量", doubles[5])
        VI.SetControlValue("dt (s)", doubles[6])
        VI.SetControlValue("控制模式", doubles[7])
    else:
...
def gen_lv_alerm_data() :
    global _alarm_index

    alarm_data_list = VI.GetControlValue("Table") # (('-309.909759', '2025/5/19 0:18', '低温警告'),)

    if not isinstance(alarm_data_list, list) or len(alarm_data_list) == 0:
        return []
    
    start_index = _alarm_index + 1
    
    # 从过去发送后的位置开始发
    if start_index >= len(alarm_data_list):
        return []
    
    unsent_data = alarm_data_list[start_index:]

    for alarm_data in unsent_data : 
        send_alarm(float(alarm_data[0]), alarm_data[1], alarm_data[2])
    
    # 更新最新数据位置
    _alarm_index = len(alarm_data_list) - 1
...
if __name__ == "__main__":
    ...
    global VI
    VI = labview.getvireference(r'C:\Users\mrd\Desktop\projects\labview_ori.vi')
